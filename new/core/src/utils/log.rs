...
#[macro_export] macro_rules! is {
	($msg:literal) => { $crate::utils::log::log_is(fmt!($msg)) };
	($var:expr)    => { $crate::utils::log::log_is(fmt!("{}", $var)) };
	($($args:tt)*) => { $crate::utils::log::log_is(fmt!($($args)*)) };
} pub use is;
...
#[macro_export] macro_rules! info {
	($msg:literal) => { $crate::utils::log::log_info(fmt!($msg)) };
	($var:expr)    => { $crate::utils::log::log_info(fmt!("{}", $var)) };
	($($args:tt)*) => { $crate::utils::log::log_info(fmt!($($args)*)) };
} pub use info;

/// Log a checkpoint that has successfully been reached.
#[macro_export] macro_rules! ok {
	($msg:literal) => { $crate::utils::log::log_ok(fmt!($msg)) };
	($var:expr)    => { $crate::utils::log::log_ok(fmt!("{}", $var)) };
	($($args:tt)*) => { $crate::utils::log::log_ok(fmt!($($args)*)) };
} pub use ok;

/// Log an error, failure or issue the user should be aware of.
#[macro_export] macro_rules! bad {
	($msg:literal) => { $crate::utils::log::log_bad(fmt!($msg)) };
	($var:expr)    => { $crate::utils::log::log_bad(fmt!("{}", $var)) };
	($($args:tt)*) => { $crate::utils::log::log_bad(fmt!($($args)*)) };
} pub use bad;

/// Log a hint to the user that may help them fix an error.
#[macro_export] macro_rules! hint {
	($msg:literal) => { $crate::utils::log::log_hint(fmt!($msg)) };
	($var:expr)    => { $crate::utils::log::log_hint(fmt!("{}", $var)) };
	($($args:tt)*) => { $crate::utils::log::log_hint(fmt!($($args)*)) };
} pub use hint;
